...
namespace margelo::nitro::nitrozlib
{
    ...
    std::shared_ptr<ArrayBuffer> NitroZlib::inflate(const std::shared_ptr<ArrayBuffer> &data)
    {
        ...
        if (ret != Z_STREAM_END)
        {
            throw std::runtime_error("Zlib inflate failed (corrupted data or wrong format)");
        }
    ...
    std::shared_ptr<ArrayBuffer> NitroZlib::deflate(const std::shared_ptr<ArrayBuffer> &data)
    {
        auto source = data->data();
        auto sourceLen = data->size();
        ...
        // If output buffer wasn't big enough (rare for deflate with estimate), loop and grow
        while (ret == Z_OK)
        {
            size_t oldSize = outBuffer.size();
